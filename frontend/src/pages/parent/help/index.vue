...
<script>
const FAQS = [
  { q: '筛查结果是医学诊断吗？', a: '不是。本产品的筛查结果基于游戏化行为数据分析，仅供家庭参考，不构成任何医学诊断意见。如有疑虑，请咨询专业医疗机构。' },
  { q: '孩子的数据安全吗？', a: '我们高度重视儿童数据安全。儿童行为数据仅用于本产品内的能力评估，不会对外共享、出售或用于商业广告目的。您可随时申请删除孩子的所有数据。' },
  { q: '筛查需要多长时间？', a: '每种游戏类型约需 5-10 分钟，建议在安静环境下完成。全部 6 种游戏类型完成后可获得综合能力图谱。' },
  { q: '风险等级高意味着什么？', a: '高风险不等于孩子"有问题"，而是提示某些能力维度需要重点关注和支持。读写障碍与智力无关，通过科学训练可以显著改善。' },
  { q: '如何让孩子配合完成筛查？', a: '建议选择孩子状态好的时间段，以"玩游戏"的方式引导，不要强调"考试"。每次筛查时间不超过15分钟，避免疲劳。' },
  ...
  { q: 'AI 问答有什么限制？', a: 'AI 助手专注于读写障碍相关问题，不提供医学诊断意见。对于高风险情况，AI 会建议寻求专业机构支持，而非给出确定性医疗判断。' },
]
...
export default {
  data() {
    return {
      ...
      expandedIndex: -1,
      quickItems: [
        { title: '筛查说明', icon: 'ph-magnifying-glass', color: 'blue', faqIndex: 0 },
